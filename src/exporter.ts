...
export class Exporter {
  ...
  private async buildExportSet(root: TFile): Promise<TFile[]> {
    ...
    if (mode === "backlinks") return this.unique([root, ...this.getBacklinks(root)]);
    if (mode === "outlinks") return this.unique([root, ...this.getOutlinks(root)]);
 ...
 private getBacklinks(root: TFile): TFile[] {
  ...
  private getOutlinks(root: TFile): TFile[] {
    ...
    for (const l of links) {
      const dest = this.app.metadataCache.getFirstLinkpathDest(l.link, root.path);
  ...
  private getGraph(root: TFile, depth: number): TFile[] {
    ...
    while (queue.length) {
      ...
      for (const l of links) {
        ...
        if (dest instanceof TFile && dest.extension === "md" && !visited.has(dest.path)) {
  ...
  private async ensurePageForFile(file: TFile): Promise<void> {
    ...
    if (mapped?.pageId && this.settings.updateExisting) {
      const updated = await this.client.updatePage(mapped.pageId, title, storage);
      this.mapping.set({
        filePath: file.path,
        pageId: updated.id,
        title,
    ...
    const found = await this.client.searchPageByTitle(this.settings.spaceKey, title);
  ...
  private rewriteWikilinksToMarkdownLinks(markdown: string, fromFile: TFile): string {
    ...
    return markdown.replace(/\[\[([^\]|#]+)(?:#[^\]|]+)?(?:\|([^\]]+))?\]\]/g, (_m, rawTarget, rawAlias) => {
      const target = String(rawTarget).trim();
      const alias = rawAlias ? String(rawAlias).trim() : target;

      // Resolve Obsidian link to a file, then look up mapping by file path
      const dest = this.app.metadataCache.getFirstLinkpathDest(target, fromFile.path);
